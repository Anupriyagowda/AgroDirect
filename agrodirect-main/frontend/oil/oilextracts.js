async function fetchOilExtracts() {
    try {
        const response = await fetch("/client/oilextracts");
            ...
            if (role === 'consumer' && localStorage.getItem('agroToken')) {
              reviewForm = `
                <div class="review-form mt-3 border-top pt-3">
                  ...
                  <button class="btn btn-sm btn-primary w-100" onclick="submitReview('${item.id}','oilextracts','${itemId}','${item.name.replace(/'/g, "\\'")}','${item.sellerEmail || 'admin@agrodirect.com'}')" data-i18n="submit">Submit Review</button>
                </div>
              `;
            }
                ...
                <div class="col">
                    <div class="card product-card">
                        ...
                        <div class="card-body text-center">
                            ...
                            <p class="product-price">₹${item.price} per bottle</p>
                        ...
                        <div class="oil-type">
                            <select class="form-select" id="type-${itemId}">
                        ...
                        <div class="unit-selector">
                            <select class="form-select" id="size-${itemId}">
...
window.submitRating = function(oilId) {
    const select = document.getElementById(`rate-${oilId}`);
...
async function removeFromCart(itemName) {
    ...
    if (!token) {
        if (window.Toast) {
            Toast.show('Please login to manage your cart', 'info');
        } else {
    ...
    try {
        ...
        // We need itemPrice here, but it's not passed to removeFromCart. 
        // In this app, we usually fetch it from the API or keep it in the closure.
        // For now, let's assume it's available or we can refresh the page.
        // Actually, in fetchOilExtracts we pass oil.price.
...
window.submitReview = async function(productId, productType, itemId, productName, sellerEmail) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            ratingSelect.value = '';
            feedbackInput.value = '';
            // Reload oil extracts to show updated rating info
            fetchOilExtracts();
        } else {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (searchBox) {
        searchBox.addEventListener('input', (e) => {
            ...
            const cards = document.querySelectorAll('#oils-container .col');
            ...
            const anyVisible = Array.from(document.querySelectorAll('#oils-container .col')).some(c => c.style.display !== 'none');
            let noResults = document.querySelector('#oils-container .no-results');
            if (!anyVisible) {
                if (!noResults) {
                    ...
                    document.getElementById('oils-container').appendChild(noResults);
                }
            } else {
