async function fetchSaplings() {
    try {
        const response = await fetch("/client/saplings");
            ...
            if (role === 'consumer' && localStorage.getItem('agroToken')) {
              reviewForm = `
                <div class="review-form mt-3 border-top pt-3">
                  ...
                  <button class="btn btn-sm btn-primary w-100" onclick="submitReview('${item.id}','saplings','${itemId}','${item.name.replace(/'/g, "\\'")}','${item.sellerEmail || 'admin@agrodirect.com'}')" data-i18n="submit">Submit Review</button>
                </div>
              `;
            }
                ...
                <div class="col">
                    <div class="card product-card">
                        ...
                        <div class="card-body text-center">
                            ...
                            <p class="product-price">₹${item.price} per sapling</p>
                        ...
                        <div class="sapling-age">
                            <select class="form-select" id="age-${itemId}">
...
window.submitRating = function(saplingId) {
    const select = document.getElementById(`rate-${saplingId}`);
...
async function addToCart(itemName, basePrice, itemImg, sellerEmail) {
    ...
    try {
        ...
        btn.onclick = () => removeFromCart(itemName, basePrice, itemImg);
...
window.submitReview = async function(productId, productType, itemId, productName, sellerEmail) {
    ...
    try {
        ...
        if (res.ok) {
            ...
            ratingSelect.value = '';
            feedbackInput.value = '';
            // Reload saplings to show updated rating info
            fetchSaplings();
        } else {
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (searchBox) {
        searchBox.addEventListener('input', (e) => {
            ...
            const cards = document.querySelectorAll('#saplings-container .col');
            ...
            const anyVisible = Array.from(document.querySelectorAll('#saplings-container .col')).some(c => c.style.display !== 'none');
            let noResults = document.querySelector('#saplings-container .no-results');
            if (!anyVisible) {
                if (!noResults) {
                    ...
                    document.getElementById('saplings-container').appendChild(noResults);
                }
            } else {
