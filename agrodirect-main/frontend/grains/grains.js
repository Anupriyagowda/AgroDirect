async function fetchGrains() {
    try {
        const response = await fetch("/client/grains");
        ...
        grainsData.forEach(item => {
            ...
            // Out of stock logic
            const stock = typeof item.stock === 'number' ? item.stock : 10; // Default 10 if not present
            let outOfStockMsg = '';
            let addCartBtn = '';
            let role = 'consumer';
            try {
            ...
            if (role === 'consumer' && localStorage.getItem('agroToken')) {
              reviewForm = `
                <div class="review-form mt-3 border-top pt-3">
                  ...
                  <button class="btn btn-sm btn-primary w-100" onclick="submitReview('${item.id}','grains','${itemId}','${item.name.replace(/'/g, "\\'")}','${item.sellerEmail || 'admin@agrodirect.com'}')" data-i18n="submit">Submit Review</button>
                </div>
              `;
            }
                ...
                <div class="col">
                    <div class="card product-card">
                        ...
                        <div class="card-body text-center">
                            ...
                            <p class="product-price">₹${item.price} per kg</p>
                        ...
                        <div class="quantity-selector">
                            ...
                            <input type="number" class="form-control quantity-input" id="quantity-${itemId}" value="1" min="0.5" step="0.5">
            ...
            `;
            grainsContainer.innerHTML += grainCard + outOfStockMsg;
...
window.submitReview = async function(productId, productType, itemId, productName, sellerEmail) {
  ...
  try {
    ...
    if (res.ok) {
      ...
      ratingSelect.value = '';
      feedbackInput.value = '';
      // Reload grains to show updated rating info
      fetchGrains();
    } else {
...
async function addToCart(itemName, itemPrice, itemImg, sellerEmail) {
    ...
    const typeId = `type-${itemName.replace(/\s+/g, '-')}`;
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    if (searchBox) {
        searchBox.addEventListener('input', (e) => {
            ...
            const cards = document.querySelectorAll('#grains-container .col');
            ...
            const anyVisible = Array.from(document.querySelectorAll('#grains-container .col')).some(c => c.style.display !== 'none');
            let noResults = document.querySelector('#grains-container .no-results');
            if (!anyVisible) {
                if (!noResults) {
                    ...
                    document.getElementById('grains-container').appendChild(noResults);
                }
            } else {
